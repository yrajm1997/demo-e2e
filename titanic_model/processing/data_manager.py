...
def read_data_file():
    ...
    import dvc.api
    import pandas as pd
    repo = 'https://' + os.environ['GH_USERNAME'] + ':' + os.environ['GH_ACCESS_TOKEN'] + '@github.com/yrajm1997/dvc-titanic-data'
    data_revision = os.environ['DATA_VERSION']
    remote_config = {
        'access_key_id': os.environ["AWS_ACCESS_KEY_ID"],
        'secret_access_key': os.environ["AWS_SECRET_ACCESS_KEY"],
    }
    with dvc.api.open('data/titanic.csv', repo=repo, rev=data_revision, remote_config=remote_config) as file:
        df = pd.read_csv(file)
    return df
